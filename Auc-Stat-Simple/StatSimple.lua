...
function lib.CommandHandler(command, ...)
	...
	if (command == "help") then
		print(_TRANS('Help for Auctioneer Advanced - Simple') )
		local line = AucAdvanced.Config.GetCommandLead(libType, libName)
		print(line, _TRANS('help}} - this Simple help') )
		print(line, _TRANS('clear}} - clear current') , myFaction, _TRANS('Simple price database') )
		print(line, _TRANS('push}} - force the') , myFaction, _TRANS('Simple daily stats to archive (start a new day)') )
	elseif (command == "clear") then
		print(_TRANS('Clearing Simple stats for').." {{", myFaction, "}}")
		private.ClearData()
...
function lib.ClearItem(hyperlink, faction, realm)
	...
	print(libType.._TRANS('- Simple: clearing data for ') ..hyperlink.._TRANS(' for').." {{"..faction.."}}")
...
function private.SetupConfigGui(gui)
	local id = gui:AddTab(lib.libName, _TRANS('Simple Modules') )

	gui:AddHelp(id, "what simple stats",
		_TRANS('What are simple stats?') ,
		_TRANS('Simple stats are the numbers that are generated by the Simple module, the Simple module averages all of the prices for items that it sees and provides moving 3, 7, and 14 day averages.  It also provides daily minimum buyout along with a running average minimum buyout within 10% variance.') 
		)

	gui:AddHelp(id, "what moving day average",
		_TRANS('What does \'moving day average\' mean?') ,
		_TRANS('Moving average means that it places more value on yesterday\'s moving averagethan today\'s average.  The determined amount is then used for tomorrow\'s moving average calculation.') 
		)

	gui:AddHelp(id, "how day average calculated",
		_TRANS('How is the moving day averages calculated exactly?') ,
		_TRANS('Todays Moving Average is ((X-1)*YesterdaysMovingAverage + TodaysAverage) / X, where X is the number of days (3,7, or 14).') 
		)

	gui:AddHelp(id, "no day saved",
		_TRANS('So you aren\'t saving a day-to-day average?') ,
		_TRANS('No, that would not only take up space, but heavy calculations on each auction house scan, and this is only a simple model.') )

	gui:AddHelp(id, "minimum buyout",
		_TRANS('Why do I need to know minimum buyout?') ,
		_TRANS('While some items will sell very well at average within 2 days, others may sell only if it is the lowest price listed.  This was an easy calculation to do, so it was put in this module.') 
		)

	gui:AddHelp(id, "average minimum buyout",
		_TRANS('What\'s the point in an average minimum buyout?') ,
		_TRANS('This way you know how good a market is dealing.  If the MBO (minimum buyout) is bigger than the average MBO, then it\'s usually a good time to sell, and if the average MBO is greater than the MBO, then it\'s a good time to buy.') 
		)

	gui:AddHelp(id, "average minimum buyout variance",
		_TRANS('What\'s the \'10% variance\' mentioned earlier for?') ,
		_TRANS('If the current MBO is inside a 10% range of the running average, the current MBO is averaged in to the running average at 50% (normal).  If the current MBO is outside the 10% range, the current MBO will only be averaged in at a 12.5% rate.') 
		)

	gui:AddHelp(id, "why have variance",
		_TRANS('What\'s the point of a variance on minimum buyout?') ,
		_TRANS('Because some people put their items on the market for rediculous price (too low or too high), so this helps keep the average from getting out of hand.') 
		)

	gui:AddHelp(id, "why multiply stack size simple",
		_TRANS('Why have the option to multiply stack size?') ,
		_TRANS('The original Stat-Simple multiplied by the stack size of the item, but some like dealing on a per-item basis.') 
		)

	gui:AddControl(id, "Header",     0,    _TRANS('Simple options') )
	gui:AddControl(id, "Note",       0, 1, nil, nil, " ")
	gui:AddControl(id, "Checkbox",   0, 1, "stat.simple.enable", _TRANS('Enable Simple Stats') )
	gui:AddTip(id, _TRANS('Allow Simple Stats to gather and return price data') )
	gui:AddControl(id, "Note",       0, 1, nil, nil, " ")

	gui:AddControl(id, "Checkbox",   0, 1, "stat.simple.tooltip", _TRANS('Show simple stats in the tooltips?') )
	gui:AddTip(id, _TRANS('Toggle display of stats from the Simple module on or off') )
	gui:AddControl(id, "Checkbox",   0, 2, "stat.simple.avg3", _TRANS('Display Moving 3 Day Average') )
	gui:AddTip(id, _TRANS('Toggle display of 3-Day average from the Simple module on or off') )
	gui:AddControl(id, "Checkbox",   0, 2, "stat.simple.avg7", _TRANS('Display Moving 7 Day Average') )
	gui:AddTip(id, _TRANS('Toggle display of 7-Day average from the Simple module on or off') )
	gui:AddControl(id, "Checkbox",   0, 2, "stat.simple.avg14", _TRANS('Display Moving 14 Day Average') )
	gui:AddTip(id,_TRANS( 'Toggle display of 14-Day average from the Simple module on or off') )
	gui:AddControl(id, "Checkbox",   0, 2, "stat.simple.minbuyout", _TRANS('Display Daily Minimum Buyout') )
	gui:AddTip(id, _TRANS('Toggle display of Minimum Buyout from the Simple module on or off') )
	gui:AddControl(id, "Checkbox",   0, 2, "stat.simple.avgmins", _TRANS('Display Average of Daily Minimum Buyouts') )
	gui:AddTip(id,_TRANS( 'Toggle display of Minimum Buyout average from the Simple module on or off') )
	gui:AddControl(id, "Note",       0, 1, nil, nil, " ")
	gui:AddControl(id, "Checkbox",   0, 1, "stat.simple.quantmul", _TRANS('Multiply by stack size') )
	gui:AddTip(id, _TRANS('Multiplies by current stack size if on') )
	gui:AddControl(id, "Checkbox",   0, 1, "stat.simple.reportsafe", _TRANS('Report safer prices for low volume items') )
