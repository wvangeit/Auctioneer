...
function lib.GetPrice(hyperlink, serverKey)
	...
	-- todo: code here to change storeProperty to "0" if user requested base item (transmog) mode
	local dailydata, meansdata = private.GetItemDataRead(serverKey, storeID, storeProperty)
	if not (dailydata or meansdata) then return end -- todo: consider fallback mode if property was not "0"?
...
function private.SetupConfigGui(gui)
	...
	function private.addTooltipControls(id)
		...
		gui:AddControl(id, "Checkbox",   0, 4, "stat.simple.tooltip", L'SIMP_Interface_Show')--Show simple stats in the tooltips?
		gui:AddTip(id, L'SIMP_HelpTooltip_Show')--Toggle display of stats from the Simple module on or off
		gui:AddControl(id, "Checkbox",   0, 6, "stat.simple.avg3", L'SIMP_Interface_Toggle3Day')--Display Moving 3 Day Average
		gui:AddTip(id, L'SIMP_HelpTooltip_Toggle3Day')--Toggle display of 3-Day average from the Simple module on or off
		gui:AddControl(id, "Checkbox",   0, 6, "stat.simple.avg7", L'SIMP_Interface_Toggle7Day')--Display Moving 7 Day Average
		gui:AddTip(id, L'SIMP_HelpTooltip_Toggle7Day')--Toggle display of 7-Day average from the Simple module on or off
		gui:AddControl(id, "Checkbox",   0, 6, "stat.simple.avg14", L'SIMP_Interface_Toggle14Day')--Display Moving 14 Day Average
		gui:AddTip(id,L'SIMP_HelpTooltip_Toggle14Day')--Toggle display of 14-Day average from the Simple module on or off
		gui:AddControl(id, "Checkbox",   0, 6, "stat.simple.minbuyout", L'SIMP_Interface_MinBuyout')--Display Daily Minimum Buyout
		...
		gui:AddControl(id, "Checkbox",   0, 6, "stat.simple.avgmins", L'SIMP_Interface_MinBuyoutAverage')--Display Average of Daily Minimum Buyouts
		...
		gui:AddControl(id, "Note",       0, 1, nil, nil, " ")
		gui:AddControl(id, "Checkbox",   0, 4, "stat.simple.quantmul", L'SIMP_Interface_MultiplyStack')--Multiply by stack size
...
function private.ProcessTooltip(tooltip, hyperlink, serverKey, quantity, decoded, additional, order)
	...
	if (seenDays + dayCount > 0) then
		...
		if (seenDays > 0) then
			...
			tooltip:AddLine("  "..L('SIMP_Tooltip_SeenNumberDays'):format(seenCount+dayCount, seenDays) ) --Seen {{%s}} over {{%s}} days:
