...
function lib.CommandHandler(command, ...)
	...
	if (command == "help") then
		print(_TRANS('Help for Auctioneer Advanced - StdDev') )
		local line = AucAdvanced.Config.GetCommandLead(libType, libName)
		print(line, _TRANS('help}} - this StdDev help') )
		print(line, _TRANS('clear}} - clear current') , myFaction, _TRANS('StdDev price database') )
	elseif (command == "clear") then
		print(_TRANS('Clearing StdDev stats for').." {{", myFaction, "}}")
		data[myFaction] = nil
	end
end
...
function private.SetupConfigGui(gui)
	...
	gui:AddHelp(id, "what stddev stats",
		_TRANS('What are StdDev stats?') ,
		_TRANS('StdDev stats are the numbers that are generated by the StdDev module consisting of a filtered Standard Deviation calculation of item cost.') 
		)

	gui:AddHelp(id, "filtered stddev",
		_TRANS('What do you mean filtered?') ,
		_TRANS('Items outside a (1.5*Standard) variance are ignored and assumed to be wrongly priced when calculating the deviation.') 
		)
	...
	gui:AddHelp(id, "what normalized",
		...
		_TRANS('In short terms again, it is the average of those values determined within the standard deviation variance calculation.') 
		)

	gui:AddHelp(id, "what confidence",
		_TRANS('What does confidence mean?') ,
		_TRANS('Confidence is a value between 0 and 1 that determines the strength of the calculations (higher the better).') 
		)

	gui:AddHelp(id, "why multiply stack size stddev",
		_TRANS('Why have the option to multiply by stack size?') ,
		_TRANS('The original Stat-StdDev multiplied by the stack size of the item, but some like dealing on a per-item basis.') 
		)

	gui:AddControl(id, "Header",     0,    "StdDev options")
	gui:AddControl(id, "Note",       0, 1, nil, nil, " ")
	gui:AddControl(id, "Checkbox",   0, 1, "stat.stddev.enable", _TRANS('Enable StdDev Stats') )
	gui:AddTip(id, _TRANS('Allow StdDev to gather and return price data') )
	gui:AddControl(id, "Note",       0, 1, nil, nil, " ")

	gui:AddControl(id, "Checkbox",   0, 1, "stat.stddev.tooltip", _TRANS('Show stddev stats in the tooltips?') )
	gui:AddTip(id, _TRANS('Toggle display of stats from the StdDev module on or off') )
	gui:AddControl(id, "Checkbox",   0, 2, "stat.stddev.mean", _TRANS('Display Mean') )
	gui:AddTip(id, _TRANS('Toggle display of \'Mean\' calculation in tooltips on or off') )
	gui:AddControl(id, "Checkbox",   0, 2, "stat.stddev.normal", _TRANS('Display Normalized') )
	gui:AddTip(id, _TRANS('Toggle display of \'Normalized\' calculation in tooltips on or off') )
	gui:AddControl(id, "Checkbox",   0, 2, "stat.stddev.stdev", _TRANS('Display Standard Deviation') )
	gui:AddTip(id,_TRANS( 'Toggle display of \'Standard Deviation\' calculation in tooltips on or off') )
	gui:AddControl(id, "Checkbox",   0, 2, "stat.stddev.confid", _TRANS('Display Confidence') )
	gui:AddTip(id,_TRANS( 'Toggle display of \'Confidence\' calculation in tooltips on or off') )
	gui:AddControl(id, "Note",       0, 1, nil, nil, " ")
	gui:AddControl(id, "Checkbox",   0, 1, "stat.stddev.quantmul", _TRANS('Multiply by Stack Size') )
...
function lib.ProcessTooltip(frame, name, hyperlink, quality, quantity, cost, ...)
	...
	if (mean and mean > 0) then
		EnhTooltip.AddLine(_TRANS('StdDev prices').." ("..count.._TRANS(' points').."):")
...
function lib.ClearItem(hyperlink, faction, realm)
	...
	if (data[faction]) then
		print(libType.._TRANS('- StdDev: clearing data for ') ..hyperlink.._TRANS(' for ').."{{"..faction.."}}")
