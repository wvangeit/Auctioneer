	...
	if (#private.inboxStart) == (#private.inboxCurrent) then --if the tables are still the same size. then look at read state
		for i in ipairs(private.inboxStart) do
			if private.inboxStart[i]["read"] ~= private.inboxCurrent[i]["read"] then
				table.insert(reconcilePending, private.inboxCurrent[i])
...
function private.PreTakeInboxMoneyHook(funcArgs, retVal, index, ignore)
	...
	if private.inboxCurrent[index]["read"] == 0 then GetInboxText(index) end--Forces message to be read.
		for i,v in pairs(reconcilePending) do
			if (reconcilePending[i]["index"] == index) then --ok lets get the invoice
				table.insert(private.Task, {["MailboxIndex"] = reconcilePending[i]["index"], ["time"] = time(), ["PendingIndex"] = i,  ["task"] = "money"})
				return "abort"
			end
		end
	end
...
function private.PreTakeInboxItemHook( ignore, retVal, index)
	...
	--Messages called by TakeInboxItem(index) "Used by mass mail mods".  the message is never read so we never have it in the pending table
	if private.inboxCurrent[index]["read"] == 0 then GetInboxText(index) end--Forces message to be read.
		for i,v in pairs(reconcilePending) do --This will only full if theres a message with an invoice
			if (reconcilePending[i]["index"] == index) then --ok lets get the invoice
				table.insert(private.Task, {["MailboxIndex"] = reconcilePending[i]["index"], ["time"] = time(), ["PendingIndex"] = i, ["task"] = "item"})
				return "abort"
			end
		end
	end
...
function private.mailonUpdate()
	if (#private.Task > 0) then
		...
		if (time() - private.Task[1]["time"] < 10) and reconcilePending[i]["retrieved"] ~= "yes" then
reconcilePending[i]["type"], reconcilePending[i]["itemName"], reconcilePending[i]["Seller/buyer"], reconcilePending[i]["bid"], reconcilePending[i]["buyout"], reconcilePending[i]["deposit"], reconcilePending[i]["fee"], reconcilePending[i]["retrieved"] = private.getInvoice(index, reconcilePending[i]["sender"],reconcilePending[i]["subject"])	
		else
			if reconcilePending[i]["retrieved"] == "no" then --we failed to get invoice 
				reconcilePending[i]["retrieved"] = "failed"
			end
	...
	elseif (#reconcilePending > 0) then		
		private.mailSort()
	end
end

function private.mailSort()
	for i,v in ipairs(reconcilePending) do
		--Get Age of message for timestamp 
		local messageAgeInSeconds = math.floor((30 - reconcilePending[i]["age"]) * 24 * 60 * 60)
		
		reconcilePending[i]["time"] = (time() - messageAgeInSeconds)
		
		--debugPrint("mail sort")
		
		if (reconcilePending[i]["sender"] == "Alliance Auction House") or (reconcilePending[i]["sender"] == "Horde Auction House") then
			if  "Auction successful: " == (string.match(reconcilePending[i]["subject"], "(Auction successful:%s)" )) and (reconcilePending[i]["retrieved"] == "yes" or reconcilePending[i]["retrieved"] == "failed") then
				...
				table.remove(reconcilePending,i)
			...
			elseif "Auction expired: " == (string.match(reconcilePending[i]["subject"], "(Auction expired:%s)" )) then
				...
				--local value = private.packString(itemName, "Auction expired", _, _, _, _, _, _, reconcilePending[i]["time"], private.wealth)
					...
					local value = private.packString(itemLink, "Auction expired", reconcilePending[i]["time"], private.wealth, date("%m-%d-%y"))
					private.databaseAdd("failedAuctions", itemID, value)
				end
				
				table.remove(reconcilePending,i)
			
			elseif "Auction won: " == (string.match(reconcilePending[i]["subject"], "(Auction won:%s)" )) and (reconcilePending[i]["retrieved"] == "yes" or reconcilePending[i]["retrieved"] == "failed") then
				...
				local  _, itemLink = private.getItemInfo(itemID, "itemid") 
					...
					local value = private.packString(itemLink, "Auction won", reconcilePending[i]["money"], reconcilePending[i]["deposit"], reconcilePending[i]["fee"], reconcilePending[i]["buyout"], reconcilePending[i]["bid"], reconcilePending[i]["Seller/buyer"], reconcilePending[i]["time"], private.wealth, date("%m-%d-%y"))				
					private.databaseAdd("completedBids/Buyouts", itemID, value)
				end				
				table.remove(reconcilePending,i)			
			
			elseif "Outbid on " == (string.match(reconcilePending[i]["subject"], "(Outbid on%s)" )) then
			...
			elseif "Sale Pending: " == (string.match(reconcilePending[i]["subject"], "(Sale Pending:%s)" )) then
			...
			debugPrint("OTHER", reconcilePending[i]["subject"])
			table.remove(reconcilePending,i)
		end
	end
end
